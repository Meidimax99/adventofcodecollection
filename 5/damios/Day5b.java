	...
	public static void main(String[] args) {
		SimpleFileReader.getInstance().readFile("advent-of-code/input5.txt", (line, count, isLast) -> {
			if (line.startsWith("[")) { // ignore first few lines for now
				...
				Stack tmp = new Stack<String>();

				for (int i = 0; i < Integer.valueOf(parts[1]); i++) {
					tmp.push(stackMap.get(Integer.valueOf(parts[3])).pop()); // stash them temporarily (in reverse order)
				}

				while (!tmp.isEmpty()) {
					stackMap.get(Integer.valueOf(parts[5])).push(tmp.pop()); // put them back (in proper order now)
				}
			}
